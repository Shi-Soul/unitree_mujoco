...
// Simple reset message structure for DDS
struct ResetMessage
{
    uint32_t timestamp;
    bool reset_flag;
...
// Reset command subscriber using Unitree SDK2
class ResetSubscriber
{
   public:
    ResetSubscriber()
    {
        ...
        std::cout << "Reset subscriber initialized. Listening on 'rt/mjc/reset' topic."
                  << std::endl;
    }

   private:
    void resetCallback(const void *msg_ptr)
    {
        if (msg_ptr)
        {
            const ResetMessage *msg = static_cast<const ResetMessage *>(msg_ptr);
            if (msg->reset_flag)
            {
                std::cout << "Reset command received via DDS, requesting robot reset" << std::endl;
                reset_requested.store(true);
            }
        }
    }

    std::unique_ptr<unitree::robot::ChannelSubscriber<ResetMessage>> reset_sub_;
};
...

        {
            ...
            if (m)
            {
                ...
                if (sim.run)
                {
                    ...
                    // misalignment condition: distance from target sim time is bigger than
                    // syncmisalign
                    bool misaligned =
                               ...
                               mj::Simulate::Clock::now() - startCPU < Seconds(refreshTime))
                        {
                            ...
                            if (sim.use_elastic_band_ == 1)
                            {
                                if (sim.elastic_band_.enable_)
                                {
                                    ...
                                    d->xfrc_applied[param::config.band_attached_link] =
                                        sim.elastic_band_.f_[0];
                                    d->xfrc_applied[param::config.band_attached_link + 1] =
                                        sim.elastic_band_.f_[1];
                                    d->xfrc_applied[param::config.band_attached_link + 2] =
...
// Create reset subscriber thread
void *ResetSubscriberThread(void *arg)
{
    ...
    while (true)
    {
        if (d)
        {
            std::cout << "Starting reset subscriber thread" << std::endl;
            break;
        }
        usleep(500000);
    }

    // Create reset subscriber
    ResetSubscriber reset_subscriber;
...
int main(int argc, char **argv)
{
    ...
    {
        param::config.robot_scene = proj_dir.parent_path() / "unitree_robots" /
                                    param::config.robot / param::config.robot_scene;
    }

    // simulate object encapsulates the UI
    auto sim = std::make_unique<mj::Simulate>(std::make_unique<mj::GlfwAdapter>(), &cam, &opt,
                                              &pert, /* is_passive = */ false);

    sim->use_elastic_band_ = param::config.enable_elastic_band;

    std::thread unitree_thread(UnitreeSdk2BridgeThread, nullptr);
    std::thread reset_thread(ResetSubscriberThread, nullptr);
